...
```{r eval=FALSE}
        radioButtons("colour","Type of histogram",choices=c("Coloured Red","Coloured Green","Coloured Blue"),selected="Red"),

```

However, as these are not valid names of colours in R, we need to do some extra processing before we can call the `hist` function. We can access the value of `input$colour` and modify accordingly. For example we could have some kind of control structure

```{r eval=FALSE}

if(input$color == "Coloured Red"){
  col <- "red"
  ....
  
} else if (input$colour == "Coloured Green"){
...
However, the following code does not create a working app. Can you work out why? `r emo::ji("thinking")`
